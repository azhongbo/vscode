...
root_fold = 'ubuntu-18.04'
iso_name  = 'ubuntu-18.04.3-desktop-amd64.iso'
vdi_file  = 'ubuntu-18.04-amd64.vdi'
iso_file  = '/sda3/VirtualBoxVMs/ISOs/' + iso_name


# 使用 時間日期格式當作版本 ver 
ver = time.strftime('%Y%m%d.%H%M',time.localtime(time.time() - 0*60*60) )
out_iso   = '/media/ubuntu-mate/E2B/_ISO/LINUX/ubuntu.1804-' + ver + '.iso'
...
e2bstartup  = '''#!/usr/bin/python3
import os

with os.popen("df -m | grep cdrom") as myCommand:
    dataArray = myCommand.readlines()

dev = dataArray[0].strip().split()[0][0:-1] + "1"

os.system("sudo mkdir /e2b")
os.system(f"sudo umount {dev}")
os.system(f"sudo mount {dev} /e2b")
os.system("sudo mkdir /e2b/startup")

os.system("sudo cp /e2b/startup/startup.sh /tmp/")
os.system("sudo chmod a+rwx /tmp/startup.sh")
os.system("/tmp/./startup.sh")
'''
fp = open("ubuntu_chroot/bin/e2b-startup.py",'w')
fp.write(e2bstartup)
fp.close
...
python3 -m pip install lxml requests beautifulsoup4 pandas numpy matplotlib plotly jupyter jupyterlab tensorflow sklearn xlrd pyautogui keras pygame opencv-python
...
yy = yy + 35
id = id + 1
actionText = str(id) + ") 安裝 Sublime/H264"
runCommand = """
cd /tmp
